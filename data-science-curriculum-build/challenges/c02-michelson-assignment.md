...
**Observations**: - Write your observations here! - I can see that the
measurements that are either 3 and 2 distinctness are closer to each
other than compared to the 1 distinctness. There are a high and close
amount of n values and the mean velocity is rounded to the same km/s. -
Why might your table differ from Michelson’s? - This table calculates
the raw data to get the mean velocities, it seems that Michelson’s data
rounded the means to the nearest 10km/s.
...
``` r
abs(true_error) > LIGHTSPEED_PM
```

    ## [1] TRUE

**Observations**: - Is Michelson’s estimate of the error (his
uncertainty) greater or less than the true error? - His estimate was
around 51km/s which is about 100km/s less than the true error of
151.542km/s - Make a quantitative comparison between Michelson’s
uncertainty and his error. - Michelson’s error was around 3 times
smaller than the actual true error. This means his true error did not
fall into the range he initially thought.
...
**Observations**: Similarities - The velocity(in vacuum) values for both
graphs span around the same values. - The central line hovers around the
dotted line that represents LIGHTSPEED_MICHELSON - Both graphs have
fluctuation and are not a simple straight line Differences - In the real
graph points measured on the same day are more clustered together while
the simulated graph has points spread out more vertically by day, this
makes sense because real data would have similar entries on the same day
due to real conditions and bias while simulated data has variation
implemented into it - The central line fluctuates higher around the
first days in the real grapg and towards the last days for the simulated
graph
...
- The velocity over temperature graph is not a straight line which shows
  the +92 correction was not enough
- The velocity against distinctness graph shows a wide spread and median
  for the poor distinctness which highlights the error tied to
  Michelsons observed readings not just noise
- The velocity against time shows a curved trend line which indicates
  slight bias and external factors outside of Michelsons control
