...
- I can observe that the price of the diamonds goes up as the carat goes
  up, which makes sense. I also notice that at each increment of carat,
  the data points line up instead of spreading out, which I would infer
  is either because of the way it is plotted or because pricing of
  diamonds is centered around each carat. I can also observe that there
  are fewer diamonds in general that are over 2.5 carats.
...
- This plot differs from the q1 plot as it adds the cut of each diamond
  which is a category of text and not a numerical value. A good way to
  incorporate this into the plot is to use color to categorize each
  point visualized. With this I can see that the fair diamonds are often
  at the lower end of the graph, higher carat cheaper cost, which makes
  sense realistically. THe opposite is also true where the more
  expensive diamonds at each carat are ideal. Of course there is a lot
  of overlap in the data but that is the main trend I can see.
